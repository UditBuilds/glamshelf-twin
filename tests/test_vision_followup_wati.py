...
class VisionFollowUpReconstruction(unittest.TestCase):
    ...
    def _run_sequence(self, wa_id, vision_result, follow_up_text, *, caption=None):
        ...
        (reply_to_image, reply_to_text) — the text actually dispatched to
        the customer on each event, or "" if nothing was sent."""
        sends = []
    ...
    def test_incident_a_photo_then_i_want_this_lashes(self):
        ...
        for run in range(1, REPEAT + 1):
            self._reset_state()
            first, second = self._run_sequence(
            ...
            if not second:
                failures.append(f"A{run}: follow-up text got no reply at all")
    ...
    def test_incident_b_photo_then_i_want_to_buy_this_lashes(self):
        ...
        for run in range(1, REPEAT + 1):
            ...
            if not second:
                failures.append(f"B{run}: follow-up text got no reply at all")
            if ASKS_FOR_PHOTO.search(second):
